...
app.post('/', async (req, res) => {
    ...
    for(let i = 0; i < 5; i++){
        ...
        prediction[3][i] =  Math.max(prediction[2][i], prediction[1][i], prediction[0][i])
    ...
    items = []
    for(let i = 0; i < 5; i++){
        items.push({'date': date, 'open': prediction[0][i], 'close': prediction[1][i], 'high': prediction[2][i], 'low': prediction[3][i], label: labels[i], 'highest': highest, 'lowest': lowest, 'average': average})
    }
